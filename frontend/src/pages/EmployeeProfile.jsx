...
export default function EmployeeProfile() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (res.data.profile_photo) {
        setPreviewUrl(`${window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://salary-management-64wa.onrender.com'}${res.data.profile_photo}`);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      await axios.post(`${window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://salary-management-64wa.onrender.com'}/api/employee-portal/profile/upload-photo`, formDataPhoto, {
        headers: { 
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      await axios.put(`${window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://salary-management-64wa.onrender.com'}/api/employee-portal/profile`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setIsEditing(false);
      fetchProfile();
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      await axios.put(`${window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://salary-management-64wa.onrender.com'}/api/employee-portal/change-password`, passData, {
        headers: { Authorization: `Bearer ${token}` }
      });
