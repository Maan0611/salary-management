  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/api/employee-portal/dashboard", {
          headers: { Authorization: `Bearer ${token}` }
        });
        setStats(res.data);
        setLoading(false);

        const attRes = await axios.get("http://localhost:5000/api/employee-portal/attendance", {
          headers: { Authorization: `Bearer ${token}` }
        });
