...
function Sidebar({ isOpen, toggleSidebar }) {
  ...
  return (
    <div 
      className={`h-full w-[250px] bg-white text-slate-600 flex flex-col shadow-xl z-40 border-r border-slate-100 flex-shrink-0 fixed lg:relative transition-all duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
    >
      ...
      {/* Brand Identity */}
      <div className={`h-24 flex items-center px-8 flex-shrink-0 relative z-10`}>
      ...
      {/* Navigation Menu */}
      <div className={`flex-1 px-6 py-8 space-y-2 overflow-y-auto relative z-10 custom-scrollbar`}>
        ...
        {menuItems.map((item) => (
          <Link 
            ...
            to={item.path} 
            className={`
              group flex items-center justify-between px-4 py-3.5 rounded-2xl font-bold transition-all duration-300 relative overflow-hidden
              ${isActive(item.path) 
      ...
      <div className={`p-6 relative z-10`}>
        <div className={`bg-slate-50 border border-slate-100 rounded-3xl p-5 group cursor-pointer hover:bg-white hover:shadow-xl transition-all`}>
